classdef Searchlight_Corr_mean2mean < CanonCorr_Base
    properties
        targetPropInVx
        targetRoiTemplate
        analyser
    ...
    methods
        function obj = Searchlight_Corr_mean2mean(opts)
            arguments
                opts (1, 1) struct
            end
            ...
            init = struct('targetRadius', [1 2 3 4], 'targetPadding', [1 1 2 3], ...
                'targetPropInVx', 1, ...
                'saveactivity', 0, 'featOnTarget', [], 'analyser', @perform_full_and_res_corr_on_searchlight.corr);
        ...
        function obj = fit(obj, nifti_image_avg, opts)
            ...
            for channel = opts.channel
                ...
                for radius = opts.radius
                    ...
                    if ~exist(center_indices_file, 'file')
                        ...
                        [~, center_indices] = searchlight(sourceRegion2D, obj.targetPropInVx, ...
                            target_mask, targetRoi2D, ...
                            obj.targetRoiTemplate{template_index}, [], mdata_only_one_channel); % for template in templates (radius range)

                        % Save results to file
                        save(center_indices_file, 'center_indices', '-v7.3');
                    end
                    ...
                    for i = 1:numel(center_indices)
                        ...
                        [pval_full{i, template_index, ch_indx}, ...
                            ...
                            corr_res_perm{i, template_index, ch_indx}] = gli2subjects( ...
                                sourceRoi, targetRoi, feature_regressors, ...
                                'vx_shuffling', vx_shuffling, 'save_activity', saveactivity_run, ...
                                'nb_feat_shuffling', feat_shuffling); % features must be passed as cell array
                    end
    ...
    methods (Static)
        ...
        function drop2nifti(info_nifti, percent_drop, center_nonempty_passing,output_file, tempalte)
            ...
            if length(center_nonempty_passing)==1
                center_nonempty_passing = [center_nonempty_passing, nan];
            end
